...
- `READONLY_PASSWORD` 或配置文件里的 `auth.password` 必须使用长度 12 到 128 字符的强密码，并同时包含大写字母、小写字母、数字和特殊字符；常见弱密码会被拒绝。启动期校验不通过时，服务端会拒绝启动，而不是只记录 warning。推荐用随机生成的强密码，例如：`export READONLY_PASSWORD="$(openssl rand -base64 24)"`。
