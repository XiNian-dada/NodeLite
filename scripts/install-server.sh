...
fetch_expected_sha256() {
  ...
  if [ ! -s "$TMP_SHA256" ]; then
    printf '%s\n' "Fetching release checksums from $checksums_url" >&2
    curl -fsSL --connect-timeout 10 --max-time 60 --max-filesize 1048576 \
      "$checksums_url" -o "$TMP_SHA256" || fail "failed to fetch release checksums"
    [ "$(wc -c <"$TMP_SHA256")" -le 1048576 ] || fail "release checksum manifest exceeds size limit"
  fi
  ...
  case "$expected_sha256" in
    ""|*[!0-9A-Fa-f]*) fail "invalid or ambiguous checksum entry for $artifact_name" ;;
  esac
  [ "${#expected_sha256}" -eq 64 ] || fail "invalid checksum length for $artifact_name"
  printf '%s' "$expected_sha256" | tr 'A-F' 'a-f'
}

# Helper scripts are executable release content and require the same trust boundary as the binary.
fetch_verified_script() {
  helper_name="$1"
  helper_path="$2"
  helper_sha256="$(fetch_expected_sha256 "$helper_name")"
  curl -fsSL --connect-timeout 10 --max-time 60 --max-filesize 1048576 \
    "$BASE_URL/$helper_name" -o "$helper_path" || fail "failed to download $helper_name"
  [ "$(wc -c <"$helper_path")" -le 1048576 ] || fail "$helper_name exceeds size limit"
  [ "$(calculate_sha256 "$helper_path")" = "$helper_sha256" ] || fail "$helper_name checksum mismatch"
}
...
mark_step "loading verified installer configuration module"
TMP_SHA256="$(mktemp "${TMPDIR:-/tmp}/nodelite-sha256.XXXXXX")"
TMP_CONFIG_HELPER="$(mktemp "${TMPDIR:-/tmp}/nodelite-config-helper.XXXXXX")"
