//! Server credential normalization and trusted proxy validation.
...
pub(in crate::config) fn validate_sha256(field: &str, value: &str) -> Result<(), ConfigError> {
...
pub(in crate::config) fn validate_totp_secret(field: &str, value: &str) -> Result<(), ConfigError> {
    validate_non_empty(field, value)?;
...
pub(in crate::config) fn uses_insecure_remote_public_base_url(public_base_url: &str) -> bool {
    uses_insecure_remote_url(public_base_url, "http")
}

pub(in crate::config) fn parse_trusted_proxies(
    values: Vec<String>,
