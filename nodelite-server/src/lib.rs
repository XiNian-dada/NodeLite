...
use clap::Parser;
use nodelite_proto::{ServerConfig, parse_server_config, uses_insecure_remote_url};
use tokio::fs;
...
mod tests {
    ...
    fn router_builds_with_v08_path_syntax() {
        let unique = SystemTime::now()
            ...
            .as_nanos();
        let registry_path =
            std::env::temp_dir().join(format!("nodelite-router-test-{unique}.json"));
    ...
    fn install_endpoints_disable_caching() {
        ...
        runtime.block_on(async {
            ...
            let unique = SystemTime::now()
                ...
                .as_nanos();
            let temp_dir =
                std::env::temp_dir().join(format!("nodelite-bootstrap-cache-test-{unique}"));
